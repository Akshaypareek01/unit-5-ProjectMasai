...
var CartValues=JSON.parse(localStorage.getItem("paticularProduct")) || [];
...
//    Storing name id in local storage after click on button
    ...
    for (var i = 0; i < btns.length; i++) {
        ...
        btns[i].addEventListener("click", function () {
			console.log("clicked");
                let productname={ProductName:elm.name, ProductID:elm.id}
                CartValues.push(productname);
                localStorage.setItem("paticularProduct", JSON.stringify(CartValues));
        });
    }
